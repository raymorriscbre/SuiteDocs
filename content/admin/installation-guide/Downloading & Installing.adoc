...
[source]
----
SUITE_PATH=/var/www/html/suitecrm/
HTTP_USER=www-data
# Set classic permissions:
mkdir cache
touch .htaccess config.php
sudo chgrp -R $HTTP_USER custom/ cache/ modules/ upload/ .htaccess config.php config_override.php
sudo chmod -R g+w custom/ cache/ modules/ upload/ .htaccess config.php config_override.php

# Set SELinux context (modern Linux distributions only)
sudo restorecon -vr ./
sudo chcon -R -t httpd_sys_rw_content_t custom/ cache/ modules/ upload/ .htaccess config.php config_override.php
sudo semanage fcontext -a -t httpd_sys_rw_content_t "$SUITE_PATH(/.*)?"
----
